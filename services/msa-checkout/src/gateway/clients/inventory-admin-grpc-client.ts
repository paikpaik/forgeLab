...
interface InventoryServiceGrpc {
  resetStock(req: { productId: string; total: number }): Observable<{ success: boolean; error: string }>;
  getStock(
...
export class InventoryAdminGrpcClient implements OnModuleInit {
  ...
  async resetStock(productId: string, total: number) {
    return firstValueFrom(this.grpcService.resetStock({ productId, total }));
  }

  async getStock(productId: string) {
    return firstValueFrom(this.grpcService.getStock({ productId }));
  }
}
