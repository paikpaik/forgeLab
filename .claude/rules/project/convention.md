...
### 실사용 페르소나 화면 + 공통 개발자 콘솔

`panel.html`의 메인 화면은 그 서비스가 흉내내는 실제 제품(웹훅 플랫폼 오퍼레이터 콘솔,
대기열 발권 화면 등)의 페르소나만 보여준다. "시스템 로그/운영 상태"처럼 실사용자는 안 보고
운영자만 보는 정보는 메인 화면에 카드로 나열하지 않고, `PanelUI.mountDevConsole({ streamUrl,
title })` 한 줄로 화면 우측 하단 플로팅 버튼 → 바텀시트(개발자 콘솔)에 몰아넣는다. 이 함수는
`@forge-lab/panel-ui`가 제공하는 공통 컴포넌트라 서비스마다 버튼/바텀시트 마크업을 직접 만들
필요가 없다. SSE 페이로드 모양이 서비스마다 다르면 `formatMessage(rawData)` 콜백으로
`{ text, atIso?, className? }`를 만들어 넘긴다(기본값은 `{ message, at }` 필드를 그대로 읽음).

webhook-relay가 이 패턴의 원형이다 — 엔드포인트 목록에 circuit 상태를 색상 dot으로 인라인
표시하고, 엔드포인트를 클릭하면 그 엔드포인트의 배달 타임라인(성공/재시도/죽은 배달+복구
버튼)이 펼쳐지는 오퍼레이터 콘솔 화면만 메인에 남기고, fan-out/replay 같은 시스템 이벤트
로그는 개발자 콘솔로 옮겼다.
